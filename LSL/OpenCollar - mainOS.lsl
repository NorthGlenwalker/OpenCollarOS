...
// ------------------------------------------------------------------------------ //
// ©   2008 - 2016  Individual Contributors and OpenCollar Official               //
// ------------------------------------------------------------------------------ //
//          http://github.com/NorthGlenwalker/OpenCollarOS                        //
// ------------------------------------------------------------------------------ //
////////////////////////////////////////////////////////////////////////////////////

string g_sCollarVersion="3.995";
...
string version_check_url = "https://raw.githubusercontent.com/NorthGlenwalker/OpenCollarIW ~version";
key github_version_request;
string news_url = "https://raw.githubusercontent.com/NorthGlenwalker/OpenCollarIW /~news";
key news_request;
...
integer compareVersions(string v1, string v2){ //compares two symantic version strings, true if v1 >= v2
    ...
    if (v1a == v2a)
    {
        if (v1Index == -1 || v2Index == -1 )//NG think this is corrct version checker
        {
            string v1b;
...
AppsMenu(key kID, integer iAuth)
{
    ...
    if (g_iTraceOn)
    {
        sPrompt +="\n Trace function is currently ON";
        sTraceButton="☒ Trace";
    }
    else
        sPrompt +="\n Trace function is currently OFF\nBy turning this ON all Owners will be notified when the collar wearer changes sims";
...
HelpMenu(key kID, integer iAuth)
{
    ...
    string sNewsButton;
    string UPDATE;
    if (g_kWearer == kID)//only for wearer
    {
        if (g_iNews)
            sNewsButton="☒ News";
        else
...
MainMenu(key kID, integer iAuth)
{
    ...
    if (g_iPaintMenu && kID == g_kWearer)
        lStaticButtons+="Paint";
    if (g_iCustomizeMenu)
        lStaticButtons+="Customize";
    if (g_iAppearanceMenu && kID == g_kWearer)
    ...
    if (kID == g_kWearer)
        lStaticButtons+=["Options","Help/About"];//make these user only now
    if (g_iLocked)
        Dialog(kID, sPrompt, "UNLOCK"+lStaticButtons, [], 0, iAuth, "Main");
    else
        Dialog(kID, sPrompt, "LOCK"+lStaticButtons, [], 0, iAuth, "Main");
}
...
integer UserCommand(integer iNum, string sStr, key kID, integer fromMenu)
{
   ...
   if (sCmd == "trace") 
    ...
    {
        if(llGetInventoryType("OpenCollar License")==INVENTORY_NOTECARD)
            llGiveInventory(kID,"OpenCollar License");
        else
            Notify(kID,"License notecard missing from collar, sorry.", FALSE); 
        if (fromMenu)
            HelpMenu(kID, iNum);
    }
    else if (sStr == "help")
    ...
    else if (sCmd == "menuto")
    {
        ...
        if (llGetAgentSize(kAv) != ZERO_VECTOR) //if kAv is an avatar in this region
        {
            if(llGetOwnerKey(kID)==kAv)
                ...
                llMessageLinked(LINK_SET, COMMAND_NOAUTH, "menu", kAv);   //else send an auth request for the menu
        }
    }
    else if (sCmd == "lock" || (!g_iLocked && sStr == "togglelock"))
    {    //does anything use togglelock?  If not, it'd be nice to get rid of it
        ...
        {   //primary owners and wearer can lock and unlock. no one else
            ...
            Notify(kID, "Sorry, only primary owners and wearer can lock the " + CTYPE + ".", FALSE);
        if (fromMenu)
            MainMenu(kID, iNum);
    }
    else if (sStr == "runaway" || sCmd == "unlock" || (g_iLocked && sStr == "togglelock"))
    {
        ...
        {  //primary owners can lock and unlock. no one else
            ...
            Notify(kID, "Sorry, only primary owners can unlock the " + CTYPE + ".", FALSE);
        if (fromMenu)
            MainMenu(kID, iNum);
    }
    else if (sCmd == "fixmenus" && kID == g_kWearer)
    {
    ...
    else if (sCmd == "update" && kID == g_kWearer)
    {
    ...
    else if (sCmd == "attachmentversion")
    {
        ...
        if (iInterfaceChannel > 0)
            iInterfaceChannel = -iInterfaceChannel;
    ...
    { // that's 28800 seconds, a.k.a. 8 hours.
       ...
       if(day==1)
       {
            ...
            else
            {
                --month;
                if(month==2)
                    day = 28+(year%4==FALSE); //To do: fix before 28th feb 2100.
                else
                    day = 30+ (!~llListFindList([4,6,9,11],[month])); //31 days hath == TRUE
             }
        }
        else
            --day;
...
default
{
    ...
    link_message(integer iSender, integer iNum, string sStr, key kID)
    {
        ...
        if (iNum == COMMAND_NOAUTH)
            return;
        else if (sStr == "504|504|504" || sStr == "504|504|504|504") // it was spamming this at one time??? so dump it
            return;
        // /SA
        ...
        { //sStr will be in form of "parent|menuname"
            ...
            {  //we only accept buttons for apps nemu
        ...
        else if (iNum == MENUNAME_REMOVE)
        {
            ...
            if (parent=="Apps" || parent=="AddOns")
            {
                ...
                if (gutiIndex != -1)
                    g_lAppsButtons = llDeleteSubList(g_lAppsButtons, gutiIndex, gutiIndex);
            }
        }
        else if (iNum == DIALOG_RESPONSE)
        {
            ...
            if (iMenuIndex != -1)
            {
                ...
                else if (sMenu=="Help/About")
                {
                    ...
                    {
                        g_kWebLookup = llHTTPRequest("https://raw.githubusercontent.com/NorthGlenwalker/OpenCollarIW /LSL/~contact", [HTTP_METHOD, "GET", HTTP_VERBOSE_THROTTLE, FALSE], "");
                        g_kCurrentUser = kAv;
                        HelpMenu(kAv, iAuth);
                    }
                        ...
                        UserCommand(iAuth,"update",kAv,TRUE);
                }
            }
        }
        ...
        else if (iNum == LM_SETTING_RESPONSE)
        {
            ...
            else if(sToken =="lock_unlocksound")
            {
                ...
                g_iListenChan = llList2Integer(llParseString2List(sValue,[","],[]),0);
            else if (sToken == "listener_safeword")
                g_sSafeWord = sValue;
        ...
        if (iChange & CHANGED_TELEPORT || iChange & CHANGED_REGION)
        {
            if (g_iTraceOn)
            {
                ...
                if (sRegionName != g_sOldRegionName)
                {
                    ...
                    string sSlurl="hop://inworldz.com:8002/"+llEscapeURL(sRegionName)+"/"+(string)llFloor(vPos.x)+"/"+(string)llFloor(vPos.y)+"/"+(string)llFloor(vPos.z);
    ...
    http_response(key id, integer status, list meta, string body)
    {
        ...
        { // be silent on failures.
            ...
            {  // strip the newline off the end of the text
                if (compareVersions(llStringTrim(body, STRING_TRIM),g_sCollarVersion))
                    g_iLatestVersion=FALSE;
                else
                    g_iLatestVersion=TRUE;
            }
            else if (id == news_request)
            {  // We got a response back from the news page on Github.  See if it's new enough to report to the user.
    ...
    timer()
    {
        ...
        {   //if no updaters responded, get upgrader info from web and remenu
            g_kWebLookup = llHTTPRequest("https://raw.githubusercontent.com/NorthGlenwalker/OpenCollarIW/LSL/~update", [HTTP_METHOD, "GET", HTTP_VERBOSE_THROTTLE, FALSE], "");
            if (g_iUpdateFromMenu)
